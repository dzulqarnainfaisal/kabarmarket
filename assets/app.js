  ...
  // ===== Deep link per kartu berita (tiap kartu punya link sendiri) =====
  function kmSlug(a){
    var s = ((a && a.title) || '').toString().toLowerCase();
    s = s.replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 80);
    return s || 'berita';
  }
  function kmFindIdxBySlug(slug){
    if(!slug) return -1;
    for(var i=0; i<articles.length; i++){ if(kmSlug(articles[i]) === slug) return i; }
    return -1;
  }
  var kmDeepLinkOpened = false;
  function kmGetBeritaParam(){
    try{ return new URLSearchParams(location.search).get('berita'); }catch(e){ return null; }
  }
  // Buka otomatis kartu yang sesuai link yang dibagikan (dipanggil setelah berita dimuat).
  function kmOpenFromUrl(){
    if(kmDeepLinkOpened) return;
    var slug = kmGetBeritaParam();
    if(!slug) return;
    var idx = kmFindIdxBySlug(slug);
    if(idx >= 0){ kmDeepLinkOpened = true; openNewsModal(idx); }
  }
  ...
  fetch('news.json', { cache: 'no-store' })
    ...
    .then(data => {
      ...
      if (Array.isArray(list) && list.length) { articles = deepCleanDash(list); renderNews(); renderHighlights(); kmOpenFromUrl(); }
    })
    .catch(() => {});
  ...
  // Tombol bagikan/salin link untuk kartu berita yang sedang dibuka.
  function kmRenderShare(a){
    var meta = document.getElementById('newsModalMeta');
    if(!meta) return;
    var box = document.getElementById('newsModalShare');
    if(!box){
      box = document.createElement('div');
      box.id = 'newsModalShare';
      box.className = 'news-share';
      meta.parentNode.insertBefore(box, meta.nextSibling);
    }
    box.innerHTML = '<button type="button" class="share-btn" id="newsShareBtn">\uD83D\uDD17 Salin link berita</button>'
      + '<a class="share-btn share-wa" id="newsShareWa" target="_blank" rel="noopener">\uD83D\uDCAC WhatsApp</a>'
      + '<span class="share-done" id="newsShareDone">\u2713 Link tersalin</span>';
    var link = location.href;
    var wa = document.getElementById('newsShareWa');
  ...
  function openNewsModal(idx){
    ...
    // Setiap kartu punya link sendiri: perbarui alamat URL agar bisa dibagikan.
    try{
      var __u = new URL(location.href);
      __u.searchParams.set('berita', kmSlug(a));
      history.replaceState({ berita: kmSlug(a) }, '', __u.toString());
    }catch(e){}
    kmRenderShare(a);
  ...
  function closeNewsModal(){
    ...
    document.body.style.overflow = '';
    // Bersihkan parameter link kartu dari alamat URL saat modal ditutup.
    try{
      var __u = new URL(location.href);
      if(__u.searchParams.has('berita')){ __u.searchParams.delete('berita'); history.replaceState(null, '', __u.pathname + __u.search + __u.hash); }
    }catch(e){}
  }
